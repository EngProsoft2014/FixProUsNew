...
public partial class CreateEstimatePage : Controls.CustomsPage
{
    ...
    #endregion
    SchEstimatesViewModel schEstimatesViewModel;

    public CreateEstimatePage(SchEstimatesViewModel model, IGenericRepository GenericRep, Services.Data.ServicesService service)
    {
        InitializeComponent();
        ORep = GenericRep;
        _service = service;
        this.BindingContext = schEstimatesViewModel = model;
    }
    ...
    private void Button_Clicked_Clear(object sender, EventArgs e)
    {
        ...
        schEstimatesViewModel.SignatureImageByte64Estimate = "";
    }

    private async void Button_Clicked_Save(object sender, EventArgs e)
    {
        var stream = await DrawBoard.GetImageStream(300, 300);
        schEstimatesViewModel.SignatureImageByte64Estimate = Convert.ToBase64String(Helpers.Utility.ReadToEnd(stream));
